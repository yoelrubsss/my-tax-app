...
import { Receipt, LogOut, User, Camera, Plus, TrendingUp, TrendingDown, DollarSign, Settings } from "lucide-react";
...
export default function Home() {
  ...
  // AI suggestions map: draft transaction id → suggested amount
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < fileArray.length; i++) {
      ...
      setUploadStatus(`מעלה קובץ ${i + 1} מתוך ${fileArray.length}: ${file.name}...`);
      ...
      try {
        ...
        if (!uploadResult.success) {
          ...
          setUploadStatus(`שגיאה בהעלאת ${file.name}: ${uploadResult.error}`);
          await new Promise((r) => setTimeout(r, 1500));
          continue;
        }

        // Step 2: AI scan the receipt
        setUploadStatus(`סורק קבלה ${i + 1} מתוך ${fileArray.length} עם בינה מלאכותית...`);
        let aiSuggestions: {
        ...
        try {
          const scanResponse = await fetch("/api/scan-receipt", {
            ...
            body: JSON.stringify({ filePath: uploadResult.path }),
          });
          const scanResult = await scanResponse.json();
          if (scanResult.success && scanResult.suggestions) {
            aiSuggestions = scanResult.suggestions;
            console.log(`🤖 AI suggestions for ${file.name}:`, aiSuggestions);
          }
        } catch (scanError) {
          console.warn("AI scan failed (non-fatal):", scanError);
        }

        // Step 3: Create DRAFT transaction — pre-fill with AI suggestions when available
        setUploadStatus(`יוצר טיוטה ${i + 1}...`);
        ...
        if (aiSuggestions) {
          if (aiSuggestions.merchant) draftPayload.merchant = aiSuggestions.merchant;
          if (aiSuggestions.date) draftPayload.date = aiSuggestions.date;
          if (aiSuggestions.category) draftPayload.category = aiSuggestions.category;
        }
        ...
        if (createResult.success) {
          successCount++;
          // Store AI-suggested amount keyed by the new transaction id
          if (aiSuggestions?.totalAmount && createResult.data?.id) {
            newSuggestions[String(createResult.data.id)] = aiSuggestions.totalAmount;
          }
        } else {
          console.error(`Draft creation failed for ${file.name}:`, createResult.error);
        }
      } catch (error) {
    ...
    // Persist AI suggestions into state
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-50 p-4 md:p-8 pb-24">
        ...
        <div className="max-w-6xl mx-auto mb-6">
          <button
            ...
            disabled={uploading}
            className="w-full md:w-auto bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 disabled:from-gray-400 disabled:to-gray-500 text-white font-bold py-4 px-8 rounded-xl shadow-lg transition-all duration-200 transform hover:scale-105 disabled:transform-none disabled:cursor-not-allowed flex items-center justify-center gap-3 text-lg"
          >
            <Camera className="w-6 h-6" />
            {uploading ? "מעבד..." : "העלאה מהירה + סריקת AI 📸"}
            <Plus className="w-6 h-6" />
          </button>
          <p className="text-center text-sm text-gray-600 mt-2">
            ניתן לבחור מספר קבלות בו-זמנית — AI יזהה את הפרטים אוטומטית 🤖
          </p>
          {uploadStatus && (
            <div className="mt-3 flex items-center justify-center gap-2 text-sm text-blue-700 bg-blue-50 border border-blue-200 rounded-lg px-4 py-2">
      ...
      <button
        onClick={handleQuickUpload}
        disabled={uploading}
        className="fixed bottom-20 left-6 md:bottom-8 md:left-8 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 disabled:from-gray-400 disabled:to-gray-500 text-white font-bold w-16 h-16 md:w-20 md:h-20 rounded-full shadow-2xl transition-all duration-200 transform hover:scale-110 disabled:transform-none disabled:cursor-not-allowed flex items-center justify-center z-40"
        title="העלאה מהירה"
      >
        {uploading ? (
